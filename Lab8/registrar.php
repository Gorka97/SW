...
$( "#email" ).keyup(function(){
	...
	xmlhttp.onreadystatechange=function(){
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200){
			...
			emailValido=xmlhttp.responseText;
			activar();
		}
	}
	xmlhttp.open("GET","clienteEmail.php?email="+email,true);
...
$( "#pass" ).keyup(function(){
	...
	xmlhttp.open("GET","clienteContrasena.php?pass="+pass,true);
	xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
	xmlhttp.send();
	...
	xmlhttp.onreadystatechange=function(){
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200){
			document.getElementById("response").innerHTML = xmlhttp.responseText;
			contrasenaValida=xmlhttp.responseText;
			activar();
		}
	}
...
function activar(){
	...
	if(emailValido== 'SI' && contrasenaValida == 'VALIDO'){
			...
			if (str_word_count($_POST["nomApe"],0)<2){
				...
				if (strcmp($_POST['pass'],$_POST['pass2']) != 0){
					...
					if(empty($_POST['email'])||empty($_POST['nomApe'])||empty($_POST['nick'])||empty($_POST['pass'])||empty($_POST['pass2'])){
						...
						$sql="INSERT INTO usuarios(email,nomApe,nick,pass,imagen) VALUES ('$_POST[email]','$_POST[nomApe]','$_POST[nick]','$passHash','$imagen')";
