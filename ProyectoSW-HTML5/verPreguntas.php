<?php
	...
	while($row = mysqli_fetch_array($datos)){
		echo '<tr> <td>'.$row['email'].'</td> <td>'.$row['pregunta'].'</td> <td>'.$row['res_correcta'].'</td> <td>'.$row['res_incorrecta1'].'</td> <td>'.$row['res_incorrecta2'].'</td> <td>'.$row['res_incorrecta3'].'</td> <td>'.$row['dificultad'].'</td> <td>'.$row['tema'].'</td> </tr>';
	}
	echo '</table>';
	$datos->close();
	mysqli_close($link);
